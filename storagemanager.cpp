...
int storagemanager::lerInsumos(Controler &ct)
{
    ifstream file;
        ...
        while(!file.eof()){
            ...
            if(aux.size()){ // Verifica se há conteúdo na linha
                ...
                for(int i=0; i<x; i++){
                    ...
                    catch(std::exception){   //Pega o erro derivado da função stoi()
                        return -1;
                    }
...
int storagemanager::salvarInsumos(Controler &ct)
{
    for(int i = 0 ; i < 28; i++){
    ...
    if(file.is_open()){
        for(int x = 0; x < 28 ; x++){ //percorre os locais um por um
            if(ct.getLocal(x).getInsumos().size() > 0){ //Se o vector de insumos do local tiver algum insumo faça:
                ...
                for(unsigned long long i = 0; i < ct.getLocal(x).getInsumos().size(); i++){
